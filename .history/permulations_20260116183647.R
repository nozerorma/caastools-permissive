...
# Update foreground and background species to only include those in pruned tree
foreground.species <- intersect(foreground.species, pruned.tree.o$tip.label)
background.species <- intersect(background.species, pruned.tree.o$tip.label)

# Recalculate sizes after pruning
foreground.size <- length(foreground.species)
background.size <- length(background.species)

# Get foreground and background values AFTER pruning
...
# Validate that we have enough species
if (foreground.size == 0) {
  stop("ERROR: No foreground species remain after tree pruning. Check that your tree and trait files have matching species.")
}
if (background.size == 0) {
  stop("ERROR: No background species remain after tree pruning. Check that your tree and trait files have matching species.")
}

write(paste("[INFO] After tree pruning: FG species =", foreground.size, "| BG species =", background.size), stdout())
