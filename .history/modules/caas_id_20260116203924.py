        ...
        if not overlap_aas:
            ...
            if not all_overlap_valid:
                ...
                if fg_changed_count < 2 and bg_changed_count < 2:
                    ...
                    print(f"\n=== CONSERVED PAIR SUMMARY ===")
                    print(f"Conserved pair set: {conserved_pair_set}")
                    print(f"Num conserved pairs: {num_conserved_pairs}, Max allowed: {max_conserved}")
                    
                    # ALWAYS record conserved pairs if they exist (for reporting)
                    if num_conserved_pairs > 0:
                        conserved_pairs_list = sorted(list(conserved_pair_set))
                        z.conserved_pairs = f"{num_conserved_pairs}:{','.join(conserved_pairs_list)}"
                        print(f"Set z.conserved_pairs = '{z.conserved_pairs}'")
                    
                    # But only pass CAAS check if within threshold
                    if num_conserved_pairs <= max_conserved:
                        z.caas = True
                        print(f"✓ CAAS PASSED (within threshold)")
                    else:
                        z.caas = False
                        print(f"✗ CAAS REJECTED (exceeds threshold)")
                    print(f"================================\n")
