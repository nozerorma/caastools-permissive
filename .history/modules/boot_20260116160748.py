...
def caasboot(processed_position, genename, list_of_traits, maxgaps_fg, maxgaps_bg, maxgaps_all, maxmiss_fg, maxmiss_bg, maxmiss_all, cycles, multiconfig, miss_pair=False, max_conserved=0, admitted_patterns=["1","2","3"]):
    """Vectorized bootstrap - processes all traits at once"""
    ...
    # VECTORIZED: Build numpy arrays for all traits at once
    n_traits = len(valid_traits)
    gaps_fg = np.array([processed_position.trait2gaps_fg[t] for t in valid_traits])
    gaps_bg = np.array([processed_position.trait2gaps_bg[t] for t in valid_traits])
    miss_fg = np.array([processed_position.trait2miss_fg[t] for t in valid_traits])
    miss_bg = np.array([processed_position.trait2miss_bg[t] for t in valid_traits])
    
    # VECTORIZED: Apply all filters as boolean masks
    mask = np.ones(n_traits, dtype=bool)
    
    if maxgaps_fg != "NO":
        mask &= (gaps_fg <= int(maxgaps_fg))
    if maxgaps_bg != "NO":
        mask &= (gaps_bg <= int(maxgaps_bg))
    if maxgaps_all != "NO":
        mask &= ((gaps_fg + gaps_bg) <= int(maxgaps_all))
    if maxmiss_fg != "NO":
        mask &= (miss_fg <= int(maxmiss_fg))
    if maxmiss_bg != "NO":
        mask &= (miss_bg <= int(maxmiss_bg))
    if maxmiss_all != "NO":
        mask &= ((miss_fg + miss_bg) <= int(maxmiss_all))
    
    # Get traits that passed filters
    filtered_traits = [valid_traits[i] for i in range(n_traits) if mask[i]]
    ...
    # Pattern check (still needs individual checking due to iscaas complexity)
    output_traits = []
    for trait in filtered_traits:
