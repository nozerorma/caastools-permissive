...
def fetch_caas(genename, processed_position, list_of_traits, output_file, maxgaps_fg, maxgaps_bg, maxgaps_all, maxmiss_fg, maxmiss_bg, maxmiss_all, multiconfig, miss_pair=False, max_conserved=0, admitted_patterns = ["1","2","3"]):
    ...
    if len(valid_traits) > 0:
        ...
        for x in valid_traits:
            # Build complete pattern following species alphabetical order
            # Sort species alphabetically, then extract their amino acids in that order
            
            # Get foreground species sorted alphabetically
            fg_species_sorted = sorted(processed_position.trait2ungapped_fg[x])
            fg_pattern_list = []
            for species in fg_species_sorted:
                aa = processed_position.d[species].split("@")[0]
                fg_pattern_list.append(aa)
            
            # Get background species sorted alphabetically
            bg_species_sorted = sorted(processed_position.trait2ungapped_bg[x])
            bg_pattern_list = []
            for species in bg_species_sorted:
                aa = processed_position.d[species].split("@")[0]
                bg_pattern_list.append(aa)
