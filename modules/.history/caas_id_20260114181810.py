    ...
    if max_conserved == 0 or not multiconfig or not multiconfig.paired_mode:
        ...
        if not overlap_aas:
            ...
            if trait and multiconfig:
                ...
                # For each overlapping amino acid, check if it comes from conserved pairs only
                for aa in overlap_aas:
                    ...
                    # Verify that every FG species with this overlapping AA has its BG pair also with this AA
                    for fg_sp in fg_with_aa:
                        ...
                        if fg_pair:
                            ...
                            if not pair_found:
                                # FG species has overlapping AA but its pair doesn't - invalid
                    ...
                    # Verify that every BG species with this overlapping AA has its FG pair also with this AA
                    for bg_sp in bg_with_aa:
                        bg_pair = multiconfig.get_pair(bg_sp)
                        if bg_pair:
                            # Check if the paired FG species has the same AA
                            pair_found = any(multiconfig.get_pair(fsp) == bg_pair for fsp in fg_with_aa)
                            
                            if not pair_found:
                                # BG species has overlapping AA but its pair doesn't - invalid
                                all_overlap_valid = False
                                break
            ...
            if not all_overlap_valid:
                # Overlap exists but not from valid conserved pairs
                z.caas = False
            else:
                # Check if there's meaningful divergence beyond the overlap
                # The non-overlapping AAs must show real change
                fg_non_overlap = set(fg) - overlap_aas
                bg_non_overlap = set(bg) - overlap_aas
                
                # If there's overlap, at least one side must have multiple unique AAs (including overlap)
                # to demonstrate real divergence, not just a single change
                has_meaningful_divergence = (len(fg) > 1 and len(fg_non_overlap) >= 1) or \
                                           (len(bg) > 1 and len(bg_non_overlap) >= 1)
                
                if not has_meaningful_divergence:
                    # Not enough divergence - just overlap or one change only
                    z.caas = False
                else:
