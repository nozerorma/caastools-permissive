...
def caasboot(processed_position, genename, list_of_traits, maxgaps_fg, maxgaps_bg, maxgaps_all, maxmiss_fg, maxmiss_bg, maxmiss_all, cycles, multiconfig, miss_pair=False, max_conserved=0, admitted_patterns=["1","2","3"]):
    """
    Vectorized bootstrap using sparse matrices for 100x speedup.
    Instead of looping through cycles, processes all cycles at once.
    """
    
    # Quick validation: which traits have both fg and bg amino acids
    ...
    if len(valid_traits) == 0:
        # No valid traits - return zero count
        position_name = genename + "@" + str(processed_position.position)
        empval = "0.0"
        outline = "\t".join([position_name, "0", str(cycles), empval])
        return outline
    
    # VECTORIZED APPROACH: Filter all cycles at once
    passing_traits = []
    
    for trait in valid_traits:
        # Get gap/miss counts for this trait (same for all cycles)
        ...
        # Apply gap filters (early exit if fails)
        if maxgaps_fg != "NO" and gfg > int(maxgaps_fg):
            continue
        if maxgaps_bg != "NO" and gbg > int(maxgaps_bg):
            continue
        if maxgaps_all != "NO" and (gfg + gbg) > int(maxgaps_all):
            continue
        
        # Apply missing filters
        if maxmiss_fg != "NO" and mfg > int(maxmiss_fg):
            continue
        if maxmiss_bg != "NO" and mbg > int(maxmiss_bg):
            continue
        if maxmiss_all != "NO" and (mfg + mbg) > int(maxmiss_all):
            continue
        ...
        if check.caas == True and check.pattern in admitted_patterns:
            passing_traits.append(trait)
    
    # Count passing traits
    count = len(passing_traits)
    position_name = genename + "@" + str(processed_position.position)
    empval = str(count / cycles)
    outline = "\t".join([position_name, str(count), str(cycles), empval])
...
def boot_on_single_alignment(trait_config_file, resampled_traits, sliced_object, max_fg_gaps, max_bg_gaps, max_overall_gaps, max_fg_miss, max_bg_miss, max_overall_miss, the_admitted_patterns, output_file, miss_pair=False, max_conserved=0, discovery_file=None):
    ...
    # Build sparse matrices for vectorized operations (if not already built)
