    ...
    if max_conserved == 0 or not multiconfig or not multiconfig.paired_mode:
        ...
        if not overlap_aas:
            ...
            if not all_overlap_valid:
                ...
                # Check total number of unique AAs across both sides
                total_unique_aas = len(set(fg).union(set(bg)))
                
                if total_unique_aas == 2:
                    # Only 2 unique AAs total - no overlap allowed at all
                    z.caas = False
                else:
                    # 3+ unique AAs - overlap is allowed if conserved pairs <= max_conserved
