...
def iscaas(input_string, multiconfig=None, position_dict=None, max_conserved=0, trait=None):
    ...
    if max_conserved == 0 or not multiconfig or not multiconfig.paired_mode:
        ...
        if not overlap_aas:
            ...
            # Check if ALL overlaps come from conserved pairs only
            conserved_pair_set = set()
            has_non_conserved_overlap = False
            ...
            if trait and multiconfig:
                ...
                # For each overlapping amino acid, verify all instances are from conserved pairs
                for aa in overlap_aas:
                    # Find which FG and BG species have this amino acid
                    ...
                    # Check each FG species with this overlapping AA
                    for fg_sp in fg_with_aa:
                        fg_pair = multiconfig.get_pair(fg_sp)
                        if fg_pair:
                            # Check if the paired BG species also has this AA
                            pair_has_aa = False
                            for bg_sp in bg_with_aa:
                                bg_pair = multiconfig.get_pair(bg_sp)
                                if bg_pair == fg_pair:
                                    # This FG species' pair also has the same AA
                                    conserved_pair_set.add(fg_pair)
                                    pair_has_aa = True
                                    break
                            
                            # If this FG species has overlap AA but its pair doesn't, reject
                            if not pair_has_aa:
                                has_non_conserved_overlap = True
                                break
                        else:
                            # FG species has no pair - overlap not from conserved pair
                            has_non_conserved_overlap = True
                            break
                    
                    if has_non_conserved_overlap:
                        break
                    
                    # Check each BG species with this overlapping AA
                    for bg_sp in bg_with_aa:
                        bg_pair = multiconfig.get_pair(bg_sp)
                        if bg_pair:
                            # Check if the paired FG species also has this AA
                            pair_has_aa = any(multiconfig.get_pair(fg_sp) == bg_pair for fg_sp in fg_with_aa)
                            
                            # If this BG species has overlap AA but its pair doesn't, reject
                            if not pair_has_aa:
                                has_non_conserved_overlap = True
                                break
                        else:
                            # BG species has no pair - overlap not from conserved pair
                            has_non_conserved_overlap = True
                            break
            ...
            # Determine if this is a valid CAAS
            if has_non_conserved_overlap:
                # There's overlap that's NOT from conserved pairs - reject
                z.caas = False
            else:
