...
def caasboot(processed_position, genename, list_of_traits, maxgaps_fg, maxgaps_bg, maxgaps_all, maxmiss_fg, maxmiss_bg, maxmiss_all, cycles, multiconfig, miss_pair=False, max_conserved=0, admitted_patterns=["1","2","3"]):
    ...
    # Convert trait lists to indices for sparse matrix operations
    if multiconfig.matrix_built:
        # Use vectorized path when sparse matrices are available
        trait_indices = [multiconfig.trait_to_idx.get(t) for t in valid_traits if t in multiconfig.trait_to_idx]
        
        if len(trait_indices) > 0:
            # Vectorized gap/missing filtering using NumPy
            trait_indices = np.array([multiconfig.trait_to_idx[t] for t in valid_traits if t in multiconfig.trait_to_idx])
            
            # Create filter masks (boolean arrays)
            valid_mask = np.ones(len(valid_traits), dtype=bool)
            
            # Apply filters vectorized across all traits
    ...
    # For now, keep the original sequential implementation
    # Further optimization will vectorize the inner loops
...
# OPTIMIZED VERSION with vectorization, chunking, progress reporting, and multiprocessing
