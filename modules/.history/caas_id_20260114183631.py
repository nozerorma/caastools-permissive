    ...
    if len(valid_traits) > 0:
        ...
        for trait in valid_traits:
            ...
            if multiconfig.paired_mode and miss_pair and trait in valid_traits:
                # Check for asymmetric missing pairs (only filter if BOTH sides have missing pairs AND they differ)
                ...
                # Only filter if both sides have missing pairs AND they're different (asymmetric)
                if miss_pairs_fg and miss_pairs_bg and miss_pairs_fg != miss_pairs_bg:
                    valid_traits.remove(trait)
                    continue
                
                # Check for asymmetric gapped pairs (only filter if BOTH sides have gapped pairs AND they differ)
                ...
                # Only filter if both sides have gapped pairs AND they're different (asymmetric)
