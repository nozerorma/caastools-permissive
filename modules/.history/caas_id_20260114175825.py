    ...
    if max_conserved == 0 or not multiconfig or not multiconfig.paired_mode:
        ...
        if not overlap_aas:
            ...
            if trait and multiconfig:
                ...
                # For each overlapping amino acid, check which pairs are conserved
                for aa in overlap_aas:
                    # Find which species have this amino acid
                    species_with_aa = [sp for sp, sp_aa in species2aa.items() if sp_aa == aa]
                    
                    # Check each FG species with this AA
                    for fg_sp in species_with_aa:
                        if fg_sp in fg_species_all:
                            fg_pair = multiconfig.get_pair(fg_sp)
                            if fg_pair:
                                # Find the BG species in the same pair
                                for bg_sp in species_with_aa:
                                    if bg_sp in bg_species_all:
                                        bg_pair = multiconfig.get_pair(bg_sp)
                                        # If both are in the same pair, it's conserved
                                        if bg_pair == fg_pair:
