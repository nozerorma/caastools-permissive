            ...
            if trait and multiconfig:
                ...
                for aa in overlap_aas:
                    ...
                    for fg_sp in fg_with_aa:
                        paired_sp = multiconfig.get_pair(fg_sp)
                        if paired_sp:
                            # Check if the paired species is in the BG group
                            if paired_sp in bg_species_all:
                                # Check if the paired BG species has the same AA
                                if species2aa.get(paired_sp) == aa:
                                    conserved_pair_set.add(multiconfig.get_pair(fg_sp))
                                else:
                    ...
                    for bg_sp in bg_with_aa:
                        paired_sp = multiconfig.get_pair(bg_sp)
                        if paired_sp:
                            # Check if the paired species is in the FG group
                            if paired_sp in fg_species_all:
...
def fetch_caas(genename, processed_position, list_of_traits, output_file, maxgaps_fg, maxgaps_bg, maxgaps_all, maxmiss_fg, maxmiss_bg, maxmiss_all, multiconfig, miss_pair=False, max_conserved=0, admitted_patterns = ["1","2","3"]):
    ...
    # DEBUG
    if len(valid_traits) > 0 and genename == "BRCA2" and processed_position.position in ["39", "411", "443", "937"]:
        print(f"DEBUG {genename}@{processed_position.position}: initial valid_traits: {valid_traits}")
    ...
    if len(valid_traits) > 0:
        ...
        for trait in valid_traits:
            ...
            if maxgaps_fg != "NO" and processed_position.trait2gaps_fg[trait] > int(maxgaps_fg):
                if len(valid_traits) > 0:
                    if genename == "BRCA2" and processed_position.position in ["39", "411", "443", "937"]:
                        print(f"DEBUG {genename}@{processed_position.position}: Removing {trait} - FG gaps {processed_position.trait2gaps_fg[trait]} > {maxgaps_fg}")
            ...
            if maxgaps_bg != "NO" and processed_position.trait2gaps_bg[trait] > int(maxgaps_bg):
                if len(valid_traits) > 0:
                    if genename == "BRCA2" and processed_position.position in ["39", "411", "443", "937"]:
                        print(f"DEBUG {genename}@{processed_position.position}: Removing {trait} - BG gaps {processed_position.trait2gaps_bg[trait]} > {maxgaps_bg}")
                    valid_traits.remove(trait)

            if maxgaps_all != "NO" and processed_position.trait2gaps_fg[trait] + processed_position.trait2gaps_bg[trait] > int(maxgaps_all):
                if len(valid_traits) > 0:
                    if genename == "BRCA2" and processed_position.position in ["39", "411", "443", "937"]:
                        print(f"DEBUG {genename}@{processed_position.position}: Removing {trait} - Total gaps {processed_position.trait2gaps_fg[trait] + processed_position.trait2gaps_bg[trait]} > {maxgaps_all}")
                    valid_traits.remove(trait)
            ...
            if maxmiss_fg != "NO" and processed_position.trait2miss_fg[trait] > int(maxmiss_fg):
                if len(valid_traits) > 0:
                    if genename == "BRCA2" and processed_position.position in ["39", "411", "443", "937"]:
                        print(f"DEBUG {genename}@{processed_position.position}: Removing {trait} - FG miss {processed_position.trait2miss_fg[trait]} > {maxmiss_fg}")
                    valid_traits.remove(trait)
            ...
            if maxmiss_bg != "NO" and processed_position.trait2miss_bg[trait] > int(maxmiss_bg):
                if len(valid_traits) > 0:
                    if genename == "BRCA2" and processed_position.position in ["39", "411", "443", "937"]:
                        print(f"DEBUG {genename}@{processed_position.position}: Removing {trait} - BG miss {processed_position.trait2miss_bg[trait]} > {maxmiss_bg}")
                    valid_traits.remove(trait)

            if maxmiss_all != "NO" and processed_position.trait2miss_fg[trait] + processed_position.trait2miss_bg[trait] > int(maxmiss_all):
                if len(valid_traits) > 0:
                    if genename == "BRCA2" and processed_position.position in ["39", "411", "443", "937"]:
                        print(f"DEBUG {genename}@{processed_position.position}: Removing {trait} - Total miss {processed_position.trait2miss_fg[trait] + processed_position.trait2miss_bg[trait]} > {maxmiss_all}")
                    valid_traits.remove(trait)
            ...
            if multiconfig.paired_mode and miss_pair and trait in valid_traits:
                if genename == "BRCA2" and processed_position.position in ["39", "411", "443", "937"]:
                    print(f"DEBUG {genename}@{processed_position.position}: Pair filtering for {trait}")
                ...
                if miss_thresholds_equal:
                    ...
                    if genename == "BRCA2" and processed_position.position in ["39", "411", "443", "937"]:
                        print(f"DEBUG {genename}@{processed_position.position}: Miss pairs FG={miss_pairs_fg}, BG={miss_pairs_bg}")
                    
                    # Check if missing pairs match
                    if miss_pairs_fg != miss_pairs_bg:
                        if genename == "BRCA2" and processed_position.position in ["39", "411", "443", "937"]:
                            print(f"DEBUG {genename}@{processed_position.position}: Removing {trait} - missing pairs don't match")
                        valid_traits.remove(trait)
                        continue
        ...
        for x in valid_traits:
            ...
            if genename == "BRCA2" and processed_position.position in ["39", "411", "443", "937"]:
                print(f"DEBUG {genename}@{processed_position.position}: Trait {x}, tag={tag}, pattern={check.pattern}, caas={check.caas}")
