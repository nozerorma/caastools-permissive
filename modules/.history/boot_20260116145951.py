...
def _process_single_position_wrapper(pos_dict):
    """Process a single position through the full pipeline (uses global worker state)"""
    global _worker_state
    ws = _worker_state
    ...
    processed = process_position(
        pos_dict,
        multiconfig=ws['resampled_traits'],
        species_in_alignment=ws['species_in_alignment']
    )
    
    # Step 2: Apply bootstrap analysis
    result = caasboot(
        processed,
        list_of_traits=ws['alltraits'],
        genename=ws['genename'],
        maxgaps_fg=ws['max_fg_gaps'],
        maxgaps_bg=ws['max_bg_gaps'],
        maxgaps_all=ws['max_overall_gaps'],
        maxmiss_fg=ws['max_fg_miss'],
        maxmiss_bg=ws['max_bg_miss'],
        maxmiss_all=ws['max_overall_miss'],
        multiconfig=ws['resampled_traits'],
        miss_pair=ws['miss_pair'],
        max_conserved=ws['max_conserved'],
        admitted_patterns=ws['admitted_patterns'],
        cycles=ws['cycles']
    )
...
def boot_on_single_alignment(trait_config_file, resampled_traits, sliced_object, max_fg_gaps, max_bg_gaps, max_overall_gaps, max_fg_miss, max_bg_miss, max_overall_miss, the_admitted_patterns, output_file, miss_pair=False, max_conserved=0, discovery_file=None, n_jobs=None):
    ...
    # Use multiprocessing Pool for parallel execution with worker initialization
    with Pool(processes=n_jobs, 
              initializer=_init_worker, 
              initargs=(resampled_traits, sliced_object.species, resampled_traits.alltraits,
                       the_genename, max_fg_gaps, max_bg_gaps, max_overall_gaps,
                       max_fg_miss, max_bg_miss, max_overall_miss, miss_pair,
                       max_conserved, the_admitted_patterns, resampled_traits.cycles)) as pool:
        # imap returns results as they complete (preserves order)
        for result_line in pool.imap(_process_single_position_wrapper, positions_list):
