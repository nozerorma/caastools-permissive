        ...
        if not overlap_aas:
            ...
            if trait and multiconfig:
                ...
                for aa in overlap_aas:
                    ...
                    # EVERY FG species with this overlapping AA must have its BG pair also with this AA
                    for fg_sp in fg_with_aa:
                        fg_pair = multiconfig.get_pair(fg_sp)
                        if not fg_pair:
                            # No pair defined for this species - cannot validate
                        ...
                        if not pair_has_aa:
                            # FG species has overlapping AA but its pair doesn't - reject
                            all_overlap_valid = False
                            break
                    ...
                    # EVERY BG species with this overlapping AA must have its FG pair also with this AA
                    for bg_sp in bg_with_aa:
                        ...
                        if not pair_has_aa:
                            # BG species has overlapping AA but its pair doesn't - reject
                            all_overlap_valid = False
                            break
            ...
            if not all_overlap_valid:
                ...
                # The non-overlapping part must have at least 2 unique AAs to show real change
                fg_non_overlap = set(fg) - overlap_aas
                bg_non_overlap = set(bg) - overlap_aas
                
                # At least one side must have 2+ unique AAs in the non-overlapping part
                has_meaningful_divergence = len(fg_non_overlap) >= 2 or len(bg_non_overlap) >= 2
                
                if not has_meaningful_divergence:
                    # Not enough divergence - need at least 2 unique AAs in non-overlap
                    z.caas = False
                else:
