...
def iscaas(input_string, multiconfig=None, position_dict=None, max_conserved=0):
    ...
    if max_conserved == 0 or not multiconfig or not multiconfig.paired_mode:
        ...
        if not overlap_aas:
            ...
            # Count how many pairs have both FG and BG members with same AA
            conserved_pair_set = set()
            
            for aa in overlap_aas:
                # Find FG species with this AA
                fg_species_with_aa = []
                bg_species_with_aa = []
                
                for species, sp_aa in species2aa.items():
                    if sp_aa == aa:
                        # Check if this species is in FG or BG
                        if species in multiconfig.s2t:
                            for trait_group in multiconfig.s2t[species]:
                                if trait_group.endswith("_1"):
                                    fg_species_with_aa.append(species)
                                elif trait_group.endswith("_0"):
                                    bg_species_with_aa.append(species)
                
                # Check which pairs are conserved (both members have the same AA)
                for fg_sp in fg_species_with_aa:
                    fg_pair = multiconfig.get_pair(fg_sp)
                    if fg_pair:
                        # Check if the paired BG species also has the same AA
                        bg_pair_species = multiconfig.pair2bg_species.get(fg_pair, [])
                        for bg_sp in bg_pair_species:
                            if bg_sp in bg_species_with_aa:
...
def fetch_caas(genename, processed_position, list_of_traits, output_file, maxgaps_fg, maxgaps_bg, maxgaps_all, maxmiss_fg, maxmiss_bg, maxmiss_all, multiconfig, miss_pair=False, max_conserved=0, admitted_patterns = ["1","2","3"]):
    ...
    if len(valid_traits) > 0:
        ...
        for x in valid_traits:
            # Build complete pattern by counting species per amino acid
            fg_aa_counts = {}
            bg_aa_counts = {}
            
            # Count amino acids in foreground species
            for species in processed_position.trait2ungapped_fg[x]:
                aa = processed_position.d[species].split("@")[0]
                fg_aa_counts[aa] = fg_aa_counts.get(aa, 0) + 1
            
            # Count amino acids in background species
            for species in processed_position.trait2ungapped_bg[x]:
                aa = processed_position.d[species].split("@")[0]
                bg_aa_counts[aa] = bg_aa_counts.get(aa, 0) + 1
            
            # Build complete pattern strings (sorted alphabetically, with repetitions)
            fg_pattern_list = []
            for aa in sorted(fg_aa_counts.keys()):
                fg_pattern_list.extend([aa] * fg_aa_counts[aa])
            
            bg_pattern_list = []
            for aa in sorted(bg_aa_counts.keys()):
                bg_pattern_list.extend([aa] * bg_aa_counts[aa])
            
            aa_tag_fg = "".join(fg_pattern_list)
            aa_tag_bg = "".join(bg_pattern_list)

            tag = "/".join([aa_tag_fg, aa_tag_bg])

            check = iscaas(tag, multiconfig, processed_position.d, max_conserved)
